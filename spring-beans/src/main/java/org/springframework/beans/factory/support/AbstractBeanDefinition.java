...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 是否抽象，来自bean属性abstract
	 */
	private boolean abstractFlag = false;

	/**
	 * 是否延时加载，对应bean的lazy-init
	 */
	...
	/**
	 * 用来标识一个bean的实例化依靠另一个bean先实例化，对应bean属性depend-on
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * autowire-candidate属性设置为false，这样容器在查找自动装配对象时，将不考虑改bean，
	 * 即它不会被考虑作为其他bean自动装配的候选者，但是改bean本事还是可以使用自动装配来注入其他的bean的。
	 */
	 ...
	 * <bean id="instanceFactoryBean" class="com.lee.InstanceFactoryBean"/>
	...
	/**
	 * 记录构造函数注入实行，对应bean属性constructor-arg
	 */
	@Nullable
